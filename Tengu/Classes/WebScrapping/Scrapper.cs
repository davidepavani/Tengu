...
namespace Tengu.Classes.WebScrapping
{
    public class Scrapper
    {
        ...
        public static Scrapper Instance
        {
            get
            {
                lock (lockObject)
        ...
        public bool InitializeDriver(out string error_message)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    WriteLog("Attempt: " + attempt + " - Is Connected to Internet: " + isConnected);
                ...
                if (!isConnected) throw new Exception("Not connected to Internet..");
        ...
        public List<AnimeData> RefreshLatestEpisodes()
        {
            List<AnimeData> animes;
            bool stale_element;

            do
            {
                animes = new List<AnimeData>();
                stale_element = false;
        ...
        #region Search
        public List<AnimeData> RefreshSearch(string pattern)
        {
            string url = _SEARCH_URL + pattern.Replace(" ", "%20");
            List<AnimeData> search_list = new List<AnimeData>();
        ...
        public CardData RefreshAnimeCard(string url)
        {
                    ...
                    if(nodesRight != null && nodesRight.Count > 0)
                    {
                        ...
                        if (nodesRight[4] != null)
                        {
                            ...
                            foreach (HtmlNode node_anime in nodesRight[4].SelectNodes("//div[@data-type='anime']/div"))
                            {
                                ...
                                if (sub_node != null)
                                {
                                    data.Episodes.Add(new AnimeData()
                                    {
                                        Title = data.Title,
        ...
        {
            Log.Instance.WriteLog("Scrapper", message);
        }
        private void WriteError(string message)
        {
            Log.Instance.WriteError("Scrapper", message);
        }
        #endregion
    }
}
